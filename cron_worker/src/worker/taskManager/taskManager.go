...
import (
	...
	"cron_worker/src/worker/scheduler"
	"encoding/json"
...
func (This *TaskManager) WatchTasks() (err error) {
	...
	// 获取etcd中/cron/tasks/目录下的所有任务，以及当前集群的revision
	Op = clientv3.OpGet(config.Cfg.BaseDir, clientv3.WithPrefix())
	if OpResp, err = This.KV.Do(context.TODO(), Op); err != nil{
		return
	}

	// 遍历Response, 并进行反序列化, 对etcd中的任务进行全量同步
	for _, kvPair = range OpResp.Get().Kvs{
		temp = &common.Task{}
		if err = json.Unmarshal(kvPair.Value, temp); err != nil {
			logrus.Infoln("WatchTasks反序列化错误...已丢弃该错误:", err.Error())
		}else{
			// 获得任务
			taskEvent = &common.TaskEvent{
	...
	go func() {
		...
		watchRespChan = This.Watcher.Watch(context.TODO(), config.Cfg.BaseDir, clientv3.WithRev(watchStartRevision), clientv3.WithPrefix())
			...
			for _, watchEvent = range watchResp.Events{
				...
				case clientv3.EventTypePut:
					...
					if err = json.Unmarshal(watchEvent.Kv.Value, task); err != nil{
						logrus.Infoln("WatchEvent反序列化错误...已丢弃该错误:", err.Error())
					}
				...
				case clientv3.EventTypeDelete:
					...
					taskBaseName = strings.TrimPrefix( string(watchEvent.Kv.Key), config.Cfg.BaseDir)
